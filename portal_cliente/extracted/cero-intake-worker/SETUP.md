...
1. Ejecutar la migración del portal:
```bash
wrangler d1 execute cero-intake --remote --file=./schema-portal.sql
```

2. Copiar `portal.html` a tu proyecto de Cloudflare Pages en `/proyecto/index.html`
...
### Cómo activar el portal para un cliente

```bash
# 1. Activar portal (genera access key + milestones default)
curl -X POST https://intake.cerostudio.ai/briefs/CS-M5X2Y/portal \
  -H "X-API-Key: TU_API_KEY"

# Respuesta:
# { "success": true, "accessKey": "abc123...", "portalUrl": "https://cerostudio.ai/proyecto/?key=abc123..." }

# 2. Enviar la portalUrl al cliente por WhatsApp/email
```

### Gestionar el proyecto desde la API

```bash
# Actualizar fase (0=Intake, 1=Diseño, 2=Sistema, 3=Desarrollo, 4=Entrega)
curl -X PATCH https://intake.cerostudio.ai/briefs/CS-M5X2Y/phase \
  -H "X-API-Key: TU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"phase": 1}'

# Marcar milestone como completado
curl -X PATCH https://intake.cerostudio.ai/milestones/3 \
  -H "X-API-Key: TU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"status": "completed"}'

# Subir archivo (cotización, contrato, etc.)
curl -X POST https://intake.cerostudio.ai/briefs/CS-M5X2Y/files \
  -H "X-API-Key: TU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"file_name": "Cotización La Cocina de María.pdf", "file_type": "quotation", "file_url": "https://drive.google.com/..."}'

# Registrar actividad
curl -X POST https://intake.cerostudio.ai/briefs/CS-M5X2Y/activity \
  -H "X-API-Key: TU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"entry_type": "delivery", "title": "Preview del sitio enviado", "description": "Link de staging compartido por WhatsApp"}'

# Actualizar pagos
curl -X PATCH https://intake.cerostudio.ai/briefs/CS-M5X2Y/payment \
  -H "X-API-Key: TU_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"total_amount": 1200, "paid_amount": 600}'

# Eliminar archivo
curl -X DELETE https://intake.cerostudio.ai/files/5 \
  -H "X-API-Key: TU_API_KEY"
```
